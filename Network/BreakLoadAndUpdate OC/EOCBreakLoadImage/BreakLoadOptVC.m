...
@interface BreakLoadOptVC ()<NSURLSessionDelegate, NSURLConnectionDataDelegate>{
...
- (void)loadImage
{
    ...
    // 去中转站找，是否有这个网络请求相关数据
    long dataPreLeght = [self backFileLegnth:self.fileName]; // 已经下载好的网络数据长度 500  bytes=500-3000 // 2500  开始位置-结束位置（结束位置没有数据说明 就没有限制）
    [request setValue:[NSString stringWithFormat:@"bytes=%ld-40000", dataPreLeght] forHTTPHeaderField:@"Range"];
    
    
    
    // 如果要做断点请求加个标识
    self.outputStream = [[NSOutputStream alloc] initToFileAtPath:FilePathInTemp(self.fileName) append:YES];
