...
{
	NSString *msg = @"Error code definition can be found in H's SecureTransport.h";
	...
	if (!done && !error && !socketEOF && hasBytesAvailable)
	{
		...
		if (flags & kSocketSecure)
		{
			...
			else
			{
				...
				// The documentation from H states:
				// 
...
- (void)cf_startTLS
{
	...
	if (!r1 && !r2) // Yes, the && is correct - workaround for H bug.
	{
