...
- (BOOL)createQueue:(AudioStreamBasicDescription)basicDescription bufferSize:(UInt32)bufferSize{
    ...
    status =  AudioQueueAddPropertyListener(_audioQueue, kAudioQueueProperty_IsRunning, EOCAudioQueuePropertyListenerProc, (__bridge void * _Nullable)(self));
    if (status != noErr) {
        NSLog(@"kAudioQueueProperty_IsRunning error");
        _audioQueue = nil;
        return NO;
    }
    ...
    AudioQueueSetProperty(_audioQueue, kAudioQueueHardwareCodecPolicy_PreferSoftware, &property, sizeof(UInt32));
...
/*
 ...
 音频文件比较小, 当已经EOF, 注意AudioQueue状态，是否是kAudioQueueProperty_IsRunning
 如果没有这个状态，直接关闭AudioQueue，导致音频没有被播放
...
- (void)handleAudioPropertyStatus:(AudioQueuePropertyID)inID{
    ...
    if(inID == kAudioQueueProperty_IsRunning){
        ...
        _running = 0;
        UInt32 size = sizeof(_running);
        AudioQueueGetProperty(_audioQueue, inID, &_running, &size);
        NSLog(@"kAudioQueueProperty_IsRunning:%d", _running);
